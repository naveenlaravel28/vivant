    ...
    <div class="container-fluid flex-grow-1 container-p-y">
        ...
        <div class="ms-auto text-end">
            <div class="my-3">
                ...
                <a href="{{ route('home') }}" class="btn btn-primary">New Packing List
                </a>
            </div>
        </div>
        <form id="reportForm" method="GET" action="{{ route('site.report.list') }}">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <a class="dropdown-item viewDetails" href="javascript:void(0);" data-value="{{ $packing->pakingDetails }}" data-id="{{ $packing->id }}" data-party="{{ $packing->customer }}">
                                    <i class="bx bx-eye-alt me-1"></i> View
                                </a>
                                @endif
                                @if($packing->status == 'pending')
                                <a class="dropdown-item editDetails" href="{{ route('order.edit', ['id' => $packing->id]) }}" data-value="{{ $packing->pakingDetails }}">
                                    <i class="bx bx-edit-alt me-1"></i> Edit
                                </a>
                                @endif
                                <a class="dropdown-item" href="{{ route('site.report.pdf', ['id' => $packing->id]) }}" target="_blank">
                                    <i class="bx bx-trash me-1"></i> PDF
                                </a>
                                <a class="dropdown-item" href="{{ route('site.report.send-pdf', ['id' => $packing->id]) }}">
                                    <i class="bx bx-trash me-1"></i> Send
                                </a>
                                <!-- <a class="dropdown-item deleteDetails" href="javascript:void(0);" data-value="{{ $packing->id }}">
...
<script type="text/javascript">
    var report_list = "{{ route('site.report.list') }}";
    ...
    function openPrintView() {
        ...
        if (printId) {
            ...
            let printUrl = `{{ route('site.report.print', ':id') }}`.replace(':id', printId);
